...
async function loadBags() {
  ...
  bags = allBags.filter(b => b.vendor_name === vendor_name);
    ...
    const res_v = await fetch(`http://tooguildtogo.onrender.com/reservations?vendor_id=${vendor_id}`);
...
async function saveEditName(){
    ...
    if(new_name !== old_name){
        const res = await fetch(`http://tooguildtogo.onrender.com/vendors/${current_id}`, {
            method: 'PATCH',
...
function loadGraphs() {
    document.getElementById('productGraph').src = 'http://tooguildtogo.onrender.com/graph/productAnalytics'
}
