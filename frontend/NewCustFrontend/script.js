...
async function searchFoodItems() {
    ...
    const res = await fetch(`http://tooguildtogo.onrender.com/bags?search=${searchTerm}`);
...
async function loadUserFavourites() {
  ...
  const res = await fetch(`http://tooguildtogo.onrender.com/customer/favourites/${userId}`);
  const data = await res.json();
...
async function filterByCategory(tag) {
   const res = await fetch(`http://tooguildtogo.onrender.com/bags?dietary_tag=${tag}`);
   const data = await res.json();
...
async function toggleFavourite(bagId, button) {
  ...
  try {
    const res = await fetch("http://tooguildtogo.onrender.com/customer/favourites", {
      method: method,
      headers: {
